...
async def get_heweather(city: str = "深圳") -> str:
    """Get he weather info."""
    payload = dict(location=city, lang="cn", unit="m", key=APIKEY,)
    try:
    ...
    now_items = ["cond_txt", "tmp", "fl", "hum", "wind_dir", "wind_spd", "wind_sc"]
    ...
    # {city}天气：{cond_txt_now}，气温：{tmp_now}℃，体感温度：{fl_now}℃。当前湿度：{hum_now}%，风力：{wind_sc_now}。
    now_str = "{}天气：{}，气温：{}℃，体感温度：{}℃，当前湿度：{}%，{}, 风速：{}千米/小时， {}。".format(
        city, *now_data
    )
    ...
    fc_header_data = ["", "日间", "晚间", "最低(度)", "最高(度)", "紫指", "湿度(%)", "风力(级)"]
